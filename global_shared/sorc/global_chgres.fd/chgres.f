...
!   2008       Moorthi S
!   2011       Added capability to work with nsst files
...
!   2015       IREDELL   : added option to write nemsio on reduced grid      
...
!
      ...
      LOGICAL use_ufo, nst_anl, grb_oro, rdgrid
...
!
     ...
     &,       NSFCI,NSLM,NLPL,NSFCO,NSSTI,NSSTO,INPTYP, kall
...
!
          call nemsio_gfs_algrd(IMO,JMO,LEVSO,NTRACO,GFSDATAO)
!
        ENDIF
...
!cggg
          allocate(ak(LEVSO+1))
          ak = VCOORD_NEW(1:(LEVSO+1),1)
          allocate(bk(LEVSO+1))
          bk = VCOORD_NEW(1:(LEVSO+1),2)
          allocate(ck(LEVSO+1))
          ck = VCOORD_NEW(1:(LEVSO+1),3)
          call checkdp(IJL,IJX,LEVSO,ak,bk,ck,
     &                 PSO,TO,QO)
...
!        CALL NEMSIO_GETHEADVAR(GFILEI,'LONB', GFSHEADI%LONB,IRET=IRET)
     ...
     &,        varr8val=GFSHEADVI%varr8val,IRET=IRET1)
        endif
     ...
     &,        aryr8len=GFSHEADVI%aryr8len,IRET=IRET1)
        endif
...
!  UPDATE NEMSIO OUTPUT FILE HEADERS USING NAME LIST VARIABLES IF NECESSARY
     ...
     &   ' input file, please define the tracers name in nemsio header!'
        ...
        IF(IVSSIG.GT.0) GFSHEADO%IVSSIG=IVSSIG
...
!if only add vvel
     ...
     &       GFSHEADVI%RECLEV(1:GFSHEADI%NREC)
          ...
          ENDDo
        ELSE
          print *,'WRONG: EXTRA tracers to output, but no field name'
     &       ,' specified!!!'
          CALL ERREXIT(26)
        ENDIF
!
...
!
     ...
     &      GFSHEADO%nmetaaryr8))
        ...
        if(  GFSHEADO%LEVS.EQ.GFSHEADI%LEVS )then
...
!     &    GFSHEADI%IDRT,GFSHEADO%IDRT
     ...
     &               GFSHEADI%IDRT, GFSHEADO%IDRT)
!       print *,'af gl2gl,after zsi',maxval(zsi),minval(zsi)
...
! start lat loop
!
        print *,'bf allocate,levs=',GFSHEADI%LEVS,'nvcoord=',
     ...
     &        GFSHEADVO%VCOORD(1:GFSHEADO%LEVS+1,1:GFSHEADO%NVCOORD,1)
!
       latloop:   DO J1=1,JMO,2*LATCH
!
          J2  = MIN(J1+2*LATCH-1,JMO)
          JL  = J2-J1+1
          IJL = IMO*JL
          IJX = IMO*JL
           print *,'in lat loop, J1=',J1,'J2=',J2,'JL=',JL,'IJX=',
     &     IJX,'bf sigio_modprd'

          CALL SIGIO_MODPRD
     &       (IJL,IJX,GFSHEADI%LEVS,GFSHEADI%NVCOORD,
     &        GFSHEADI%IDVC,GFSHEADI%IDSL,VCOORDI,IRET,
     &        PS=PSI(:,J1:J2),T=TIV(:,J1:J2,:),
     &        PM=PI(:,J1:J2,:))
!cggg
          TI = TIV / (1.+(461.50/287.05-1)*QI(:,:,:,1))  ! virtual temperature
!
          if(any(TI(:,j1:j2,:) == 0)) then
          print *,'tmp=0,j1=',j1,'j2=',j2,'tmp=',maxval(TI(:,j1:j2,:)),
     &        minval(TI(:,j1:j2,:))
           endif
           CALL NEWPS(IJL,ZSI(:,J1:J2),PSI(:,J1:J2),IJX,GFSHEADI%LEVS,
     &                 PI(:,J1:J2,:),TI(:,J1:J2,:),QI(:,J1:J2,:,:),
     &                 ZSO(:,J1:J2),PSO(:,J1:J2))
...
!    &                 GFSHEADVO%RI,GFSHEADVO%CPI,NTRACM,
     &                 PI(:,J1:J2,:),TI(:,J1:J2,:),QI(:,J1:J2,:,:),
     &                 PSO(:,J1:J2),PO(:,J1:J2,:),DPO(:,J1:J2,:))
!cggg
            call instrument(9,kall,ttot,tmin,tmax)

            CALL VINTG(IJL,IJX,GFSHEADI%LEVS,GFSHEADO%LEVS,NTRACM,
     &                 PI(:,J1:J2,:),UI(:,J1:J2,:),VI(:,J1:J2,:),
     &                 TI(:,J1:J2,:),QI(:,J1:J2,:,:),WI(:,J1:J2,:),
     &                 PO(:,J1:J2,:),UO(:,J1:J2,:),VO(:,J1:J2,:),
     &                 TO(:,J1:J2,:),QO(:,J1:J2,:,:),DTDPO(:,J1:J2,:),
     &                 WO(:,J1:J2,:))
!
!--------------------------------------------------------------------------
...
!$omp parallel do private(i,j)
          DO J=J1,J2
            DO I=1,IMO
              GFSDATAO%ZS(I,J) = ZSO(I,J)
              GFSDATAO%PS(I,J) = PSO(I,J)
            ENDDO
          ENDDO
!$omp parallel do private(i,j,k,n)
          DO K=1,GFSHEADO%LEVS
            DO J=J1,J2
              DO I=1,IMO
                GFSDATAO%DP(I,J,K) = DPO(I,J,K)
                GFSDATAO%P(I,J,K)  = PO(I,J,K)
                GFSDATAO%T(I,J,K)  = TO(I,J,K)
                GFSDATAO%U(I,J,K)  = UO(I,J,K)
                GFSDATAO%V(I,J,K)  = VO(I,J,K)
                DO N=1,NTRACO
                  GFSDATAO%Q(I,J,K,N) = QO(I,J,K,N)
                ENDDO
                GFSDATAO%W(I,J,K)  = WO(I,J,K)
              ENDDO
            ENDDO
          ENDDO
         ENDIF
...
!----------------------------------------------------
        ENDIF mquicknems
!
        DEALLOCATE(GFSDATAI%ZS, GFSDATAI%PS, GFSDATAI%P
     &,            GFSDATAI%DP, GFSDATAI%T,  GFSDATAI%U
     &,            GFSDATAI%V,  GFSDATAI%Q,  GFSDATAI%W)
!
...
!
      OUTSIG=NSIGO>0.and.(OUTTYP.LE.2.OR.OUTTYP.GE.0).AND.
     &   (INPTYP==2.and.MQUICK.EQ.0 .or.INPTYP==1)
      IF(OUTSIG) THEN
!
...
!     &   ,to(1,1:20,2),'q=',qo(1,1:10,2,1),'IMO=',IMO,'JMO=',JMO
...
!set recname, reclevtyp,reclev
         IF (INPTYP==2) THEN
           ...
           GFSHEADVO%RECNAME(3:(2+LEVSO))='dpres'
           GFSHEADVO%RECNAME((3+LEVSO):(2+2*LEVSO))='pres'
           GFSHEADVO%RECNAME((3+2*LEVSO):(2+3*LEVSO))='ugrd'
           GFSHEADVO%RECNAME((3+3*LEVSO):(2+4*LEVSO))='vgrd'
           GFSHEADVO%RECNAME((3+4*LEVSO):(2+5*LEVSO))='tmp'
           jrec=2+5*LEVSO
...
!nemsio data
     ...
     &    maxval(gfsdatao%Q(:,:,1,3)),minval(gfsdatao%Q(:,:,1,3))
        ...
        print *,'after nemsio_close gfileo,iret=',iret
!
!end OUTTYP
        ENDIF
...
!    &,                                                         4,IDRT)
     ...
     &,                                                         4,IDRT)
        CALL GL2GL(2,1,GFSDATAI%UUSTAR,IMI,JMI,GFSDATAO%UUSTAR,IMO,JMO
     &,                                                         4,IDRT)
...
!       IF(SFCHEADI%IVS.NE.200501.AND.SFCHEADO%IVS.EQ.200501)THEN
     ...
     &,                                                         4,IDRT)
         CALL GL2GL(2,1,GFSDATAI%SRFLAG,IMI,JMI,GFSDATAO%SRFLAG,IMO,JMO
     &,                                                         4,IDRT)
        END IF
...
!
            CALL SFCIO_SWHEAD(NSFCO,SFCHEADO,IRET)
            print *,'after sfcio_swhead'
            CALL SFCIO_SWDBTA(NSFCO,SFCHEADO,SFCDATAO,IRET)
            print *,'after sfcio_sclose'
...
!
         DO i=1,GFSHEADO%nmetaaryi
           if(GFSHEADVO%aryiname(i)=='lpl'.and. GFSHEADVO%aryilen(i)
     &      .ne.(GFSHEADO%LATB+1)/2)  then
     ...
     &          GFSHEADO%nmetaaryi))
         ...
         ENDDo
         DO i=1,GFSHEADO%nmetaaryi
           if(GFSHEADVO%aryiname(i)=='lpl') then
...
!
!      print *,'output nemsio chgres.out.sfn,nrec=',GFSHEADO%NREC,
!     & 'size(recname)=',size(GFSHEADVO%RECNAME),'nfminute=',
!     &   GFSHEADO%NFMINUTE
...
CC$$$
      implicit none
      INTEGER, INTENT(IN) :: IM1,JM1,IM2,JM2,KM,IP
...
C - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      KGDS2=0
      KGDS2(1)=4;KGDS2(4)=90000;KGDS2(7)=-90000
      IPOPT=0
