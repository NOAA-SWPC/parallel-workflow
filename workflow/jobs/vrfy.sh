...
#
export yzdir=${yzdir:-$BASEDIR/wx20yz}
...
##$NHCSUBSETSH $COMROT $NHCOUTPUT $NHCSUBSET_LIST $CDATE $CDUMP 120
  ...
  export NHCSUBSET_PGB=pgbq
  export NHCSUBSET_FHOUT=6
  export NHCSUBSET_FHMAX=180
  export PGB_FORMAT=2
...
#
#   Create lat-lon binary files of siganl and sigges for making analysis increment maps
#
#---------------------------
if [ $CDUMP = gdas  -a ${SIGBIN:-NO} = YES ] ; then
#---------------------------
s2g=${s2g:-$vsdbhome/nwprod/util/exec/ss2ggx}
JCAP_bin=${JCAP_bin:-254}    ;#output binary file resolution
if [ $JCAP_bin = 1534 ]; then
 lonbx=3072; latbx=1536
elif [ $JCAP_bin = 1148 ]; then
 lonbx=2304; latbx=1152
elif [ $JCAP_bin = 574 ]; then
 lonbx=1760; latbx=880
elif [ $JCAP_bin = 382 ]; then
 lonbx=1152; latbx=576
 ...
 echo " JCAP_bin=$JCAP_bin not supported, default to T254"
fi
lonbx=${lonbx:-768};  latbx=${latbx:-384}
idrtx=0  ;##0->lat-lon grid

export OMP_NUM_THREADS=4
export MP_SHARED_MEMORY=no
export KMP_STACKSIZE=2048m

ina=$ROTDIR/siganl.$CDUMP.$CDATE
ing=$ROTDIR/sigges.$CDUMP.$CDATE
oua=${ARCDIR}/siganl.${CDUMP}.${CDATE}.${lonbx}.${latbx}.bin
oug=${ARCDIR}/sigges.${CDUMP}.${CDATE}.${lonbx}.${latbx}.bin
if [ -s $ina ]; then $s2g $ina $oua tmpa.ctl $idrtx ${lonbx} ${latbx} ;fi
if [ -s $ing ]; then $s2g $ing $oug tmpg.ctl $idrtx ${lonbx} ${latbx} ;fi
#---------------------------
fi
#---------------------------
