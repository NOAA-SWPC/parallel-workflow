...
export REGRIDMOM4SH=${REGRIDMOM4SH:-$BASEDIR/ush/regrid_mom4.sh}
...
export FHDFI=$(eval echo \${FHDFIFCST$cycle$cdump:-${FHDFI:-0}}|cut -f$nknd -d,)
...
#
export FHROT=00
if [ -f $ROTDIR/grdr1 -a -f $ROTDIR/grdr2 -a -f $ROTDIR/sigr1 -a -f $ROTDIR/sigr2 -a -f $ROTDIR/sfcr ]; then
  export GRDI=$ROTDIR/grdr1
  export GRDI2=$ROTDIR/grdr2
  export SIGI=$ROTDIR/sigr1
  export SIGI2=$ROTDIR/sigr2
  export SFCI=$ROTDIR/sfcr
else
  export SIGI=$ROTDIR/$SIGISUF.$CDUMP.$CDATE
  export GRDI=$ROTDIR/$SIGISUF.$CDUMP.$CDATE
  export SFCI=$ROTDIR/$SFCISUF.$CDUMP.$CDATE
fi
if [ $use_gfs_nemsio = .true. -a $NEMSIO_IN = .true. ] ; then
  export FHINI=${FHINI:-`$nemsioget $GRDI nfhour |grep -i "nfhour"|awk -F" " '{print $2}'`}
else
  export FHINI=${FHINI:-`$SIGHDR $SIGI ifhr`}
fi
# ------------------------ post on quilt settings ----------------
#
if [[ $WRITE_DOPOST = '.true.' ]] then
 if [[ $POST_GRIBVERSION = 'grib1' ]] ; then
   ...
   export CTLFILE=$PARMPOST/postcntrl_gfs.xml
   ln -sf $CTLFILE postcntrl.xml
 fi
 ln -sf griddef.out fort.110
 if [ $SCHEDULER = 'loadleveler' ]; then
   cp /nwprod/parm/nam_micro_lookup.dat $DATATMP/eta_micro_lookup.dat
 elif [ $SCHEDULER = 'pbs' ]; then
   cp /meso/noscrub/wx20rv/REGRESSION_TEST/data_POST/eta_micro_lookup.dat $DATATMP/eta_micro_lookup.dat
...
# Are we doing IAU, and is this 4D?
export l4densvar=${l4densvar:-.false.}
export DOIAU=${DOIAU:-NO}
if [ $DOIAU = YES -a $COUP_FCST = YES ]; then
 echo "IAU is not adapted for COUPLED FCST, exiting!"
 $PERR
 exit 123
fi

# Turn off IAU in all forecast segments
# other than first segment
if [ $nknd -gt 1 ]; then
  export DOIAU=NO
fi

# If doing IAU; disable dfi (set to 0), add 6h to FHMAX
if [ $DOIAU = YES ]; then
 export iau_freq=${iau_freq:-1}
 export FHINI=3
 export FHBAK=3
 export FHMAX=$((10#$FHMAX+10#6))
 export FHDFI=0
fi
...
if [ $ENS_NUM = 1 ]; then PE1=0; fi

export NUMTHRD=${NUMTHRD:-$(eval echo \${NTHRFCST$cycle$cdump:-$nth_f1}|cut -f$nknd -d,)}
export PE1=${PE1:-$tasks}
export nodes=$((tasks*NUMTHRD/npe_node_f))
#
...
if [ $((nodes*npe_node_f/NUMTHRD)) -lt $tasks ] ; then
...
if [ $IDVC = 1 ] ; then
 ...
 export SIGLEVEL1=${SIGLEVEL1:-$FIXgsm/global_siglevel.l$LEVS.txt}
elif [ $IDVC = 2 ] ; then
 export IDVM=1 ; export IDSL=1 ; export nvcoord=2
 export SIGLEVEL2=${SIGLEVEL2:-$FIXgsm/global_hyblev.l$LEVS.txt}
elif [ $IDVC = 3 ] ; then
#export IDVM=2 ; export IDSL=2 ; export nvcoord=3 
 export ivssig=200509
 Apercent=${Apercent:-100}
 if [ $Apercent -lt 100 ] ; then
  export SIGLEVEL3=${SIGLEVEL3:-$FIXgsm/global_hyblev3.ipa$Apercent.l$LEVS.txt}
 else
  export SIGLEVEL3=${SIGLEVEL3:-$FIXgsm/global_hyblev3.l$LEVS.txt}
 fi
#
...
if [ $FHINI -eq 0 -o $FHBAK -eq 0 ] ; then
  export SIGI=${SIGI:-$SIGIND/${SIGISUF}${SUFIN}}
  export SIGI2=/dev/null
  export SFCI=${SFCI:-$SIGIND/${SFCISUF}${SUFIN}}
  if [ $NST_FCST -gt 0 ] ; then
    export NSTI=$SIGIND/${NSTISUF}${SUFIN}
  fi
  if [ $NTRAN -lt 2 ] ; then export NTRAN=2 ; fi
  if [ -s $SIGI -a -s $SFCI ] ; then
   if [ $COUP_FCST = YES ] ; then
    FDATE=$CDATE
    export OCNI=$COMROT/${OCNISUF}${SUFING}.tar
    if [ ! -s $OCNI -a $nknd -gt 1 ] ; then
     ${NCP:-/bin/cp} $COMROT/${OCNISUF}${SUFING}.tar $OCNI
    fi
   fi
  else
   export SIGI=${SIGI:-$SIGIND/${SIGISUF}${SUFIN}}
  ...
  export sig_chg=$SIGIND/$SIGISUF
  export sfc_chg=$SIGIND/$SFCISUF
  if [ $NST_FCST -gt 0 ] ; then
    export nst_chg=$SIGIND/$NSTISUF
  fi
fi
  ...
  if [ $FHSTRT = 9999999 ] ; then
    ...
    if [ -s $sigr1file ] ; then
      if [ $NEMSIO_IN = .true. ] ; then
        jcap=$($nemsioget $sigr1file jcap |grep -i "jcap" |awk -F"= " '{print $2}' |awk -F" " '{print $1}')
      else
      ...
      if [ $JCAP -ne $jcap ] ; then
        ...
        if [ $NEMSIO_IN = .true. ] ; then
          FHREST=`$nemsioget $sigr1file nfhour | grep -i "nfhour" | awk -F"= " '{print $2}' | awk -F" " '{print $1}'`
        else
    ...
    if [ -s $RESDIR/sig1r${FHSTRT}${SUFIN} ] ; then
      if [ $NEMSIO_IN = .true. ] ; then
        jcap=$($nemsioget $$RESDIR/sig1r${FHSTRT}${SUFIN} jcap |grep -i "jcap" |awk -F"= " '{print $2}' |awk -F" " '{print $1}')
      else
...
if [ $USE_RESTART = YES ] ; then
  ...
  if [ $NEMSIO_IN = .true. ] ; then
    FHREST_TMP=$($nemsioget $RESDIR/sig1r${FHMAX}${SUFIN} nfhour | grep -i "nfhour" | awk -F"= " '{print $2}' | awk -F" " '{print $1}')
  else
    FHREST_TMP=$($SIGHDR $RESDIR/sig1r${FHMAX}${SUFIN} ifhr)
  fi
  if [ -s $RESDIR/sig1r${FHREST}${SUFIN} ] ; then
    ...
    if [ $NTRAN -lt 2 ] ; then export NTRAN=2 ; fi
  elif [ $FHREST_TMP -eq $FHREST ] ; then
  ...
  if [ $DOIAU = YES ] ; then
    ...
    export GDATE=$($NDATE -$FHINI $GDATE)
    export SUFIN=.$GDUMP.$GDATE
    export SIGI=$SIGIND/${SIGOSUF}f$FHBAK$SUFIN
    export SIGI2=/dev/null
    export SFCI=$SIGIND/${SFCOSUF}a$FHBAK.$CDUMP.$CDATE
    [[ $NST_FCST -gt 0 ]]&&export NSTI=$SIGIND/${NSTOSUF}a$FHBAK.$CDUMP.$CDATE

    SUFIAU_ANL=.$CDUMP.$CDATE

    if [ $l4densvar = .true. ] ; then
      iaufhrs=""
      iaufiles_fg=""
      iaufiles_anl=""
      for f in $( seq 9 -$iau_freq 3 ); do
        iaufhrs=$f,$iaufhrs
        fhrchar=`printf %02d $f`
        iaufiles_fg=\'$SIGIND/${SIGOSUF}f$fhrchar$SUFIN\',$iaufiles_fg
        [[ $f = 6 ]]&&fhrchar="nl"
        iaufiles_anl=\'$SIGIND/${SIGOSUF}a$fhrchar$SUFIAU_ANL\',$iaufiles_anl
      done
    else
      iaufhrs=6
      iaufiles_fg=\'$SIGIND/${SIGOSUF}f06$SUFIN\'
      iaufiles_anl=\'$SIGIND/${SIGISUF}anl$SUFIAU_ANL\'
    fi
    export FCSTVARS="$FCSTVARS,iau=.true.,iau_delthrs=6,iaufhrs=$iaufhrs,iaufiles_fg=$iaufiles_fg,iaufiles_anl=$iaufiles_anl"
    ...
    export SIGI=$SIGIND/${SIGOSUF}f$FHBAK$SUFIN
    export SIGI2=/dev/null
    export SFCI=$SIGIND/${SFCOSUF}f$FHBAK$SUFIN
  ...
  if [ $NST_FCST -gt 0 ] ; then
    export NSTI=$SIGIND/${NSTOSUF}f$FHBAK$SUFIN
...
if [ $use_gfs_nemsio = .true. -a $NEMSIO_IN = .true. ] ; then
  idvc=$($nemsioget $SIGI idvc |grep -i "idvc" |awk -F"= " '{print $2}' |awk -F" " '{print $1}')
  idvm=$($nemsioget $SIGI idvm |grep -i "idvm" |awk -F"= " '{print $2}' |awk -F" " '{print $1}')
  jcap=$($nemsioget $SIGI jcap |grep -i "jcap" |awk -F"= " '{print $2}' |awk -F" " '{print $1}')
  levs=$($nemsioget $SIGI levs |grep -i "levs" |awk -F"= " '{print $2}' |awk -F" " '{print $1}')
else
  idvc=$($SIGHDR $SIGI idvc)
  idvm=$($SIGHDR $SIGI idvm)
...
#  newname=$(ls -rt ${CDATE}* | cut 11-)
  ...
  if [ $FHINI -gt 0 ] ; then
   ...
   if [ $im_om -eq 720 -a $omres = 1x1 ] ; then
     ...
     OCNINP=$OCNI
     OCNOUT=$COMROT/${OCNISUF}${SUFOUTG}.tar
...
if [[ $NTRAN -eq 0 || $IDVC -ne $idvc || $JCAP -ne $jcap || $LEVS -ne $levs ]] ; then
  ...
  export SFCINP=$SFCI
  if [ $NEMSIO_IN = .true. ] ; then
    export GFSOUT=${sig_chg}$SUFOUT
    export SFNOUT=${sfc_chg}$SUFOUT
    if [ $SIGINP = $GFSOUT -o $SFCINP = $SFNOUT ] ; then
      export GFSOUT=$DATA/$(basename $GFSOUT)
      export SFNOUT=$DATA/$(basename $SFNOUT)
    fi
  ...
  LATCH=${LATCH:-48}
  export OUTTYP_CH=${OUTTYP_CH:-1}
  if [ $CHGRESVARS = "" ]; then
    export CHGRESVARS="IDVC=$IDVC,IVSSIG=$ivssig,IVSSFC=$ivssfc,NVCOORD=$nvcoord,IDVM=$IDVM,IDSL=$IDSL,LATCH=$LATCH,OUTTYP=$OUTTYP_CH"
  fi
  if [ $IDVM -gt 3 ] ; then
...
#
    ...
    USE_RESTART=YES
#
    if [ $idvc -eq 3 -a $idvm -eq 32 ] ; then
