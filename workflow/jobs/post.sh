...
if [ $machine = WCOSS ]; then
  ...
  export LD_LIBRARY_PATH=/scratch3/NCEPDEV/nwprod/lib:$LD_LIBRARY_PATH
fi
#module load ibmpe ics lsf
export LD_LIBRARY_PATH=/nwprod/lib:$LD_LIBRARY_PATH
...
if [ $NCEPPOST = YES ] ; then
 if [ $GRIBVERSION = grib1 ]; then
  export POSTGPSH=${POSTGPSH_NP:-${POSTGPSH:-$BASE_POST/ush/global_nceppost.sh}}
  export POSTGPEXEC=${POSTGPEXEC_NP:-${POSTGPEXEC:-$BASEDIR/exec/ncep_post}}
  export PARMPOST=${PARMPOST:-$BASE_POST/parm}
  export CAT_FLX_TO_PGB=${CAT_FLX_TO_PGB:-YES}
  export OUTTYP=${OUTTYP:-1}
  export CHGRESEXEC=${CHGRESEXEC:-$BASEDIR/exec/global_chgres}
  export SIGHDR=${SIGHDR:-$NWPROD/exec/global_sighdr}
  export USHGLOBAL=${USHGLOBAL:-$USHDIR}
 else
  export OVERPARMEXEC=$BASE_POST/exec/overparm_grib
  export ANOMCATSH=$BASE_POST/ush/global_anomcat.sh
  export GENPSICHIEXE=$BASE_POST/exec/genpsiandchi
  export POSTGPSH=${POSTGPSH_NP:-${POSTGPSH:-$BASE_POST/ush/global_nceppost_grb2.sh}}
  export POSTGPEXEC=${POSTGPEXEC_NP:-${POSTGPEXEC:-$BASEDIR/exec/ncep_post}}
  export CAT_FLX_TO_PGB=${CAT_FLX_TO_PGB:-YES}
  ...
  export CTLFILEGFSANL=${PARMPOST}/postcntrl_gfs_anl.xml
  export CTLFILEGFS=${PARMPOST}/postcntrl_gfs.xml
  export CTLFILEGFS_f00=${PARMPOST}/postcntrl_gfs_f00.xml
  export CTLFILEGOES=${PARMPOST}/postcntrl_gfs_goes.xml
 fi
else
 export POSTGPSH=${POSTGPSH_GP:-${POSTGPSH:-$USHDIR/global_postgp.sh}}
 export POSTGPEXEC=${POSTGPEXEC_GP:-${POSTGPEXEC:-$BASEDIR/exec/global_postgp}}
...
export SMARTPRECIPSH=${SMARTPRECIPSH:-$BASEDIR/ush/gfs_smart_precip.sh}
#export GFS_SMARTMAKEPRECIPEXEC=${GFS_SMARTMAKEPRECIPEXEC:-$BASEDIR/exec/gfs_smartmakeprecip}
...
if [ $FHINI -eq 0 -a -s $SIGIND/${SIGISUF}$SUFOUT ] ; then
 if [ $NCEPPOST = YES ] ; then
   ...
   if [ $GRIBVERSION = grib1 ]; then
    ...
    export CTLFILE1=${CTLFILEGFSANL:-$PARMPOST/postcntrl_gfs_anl.xml}
...
#    export POSTGPLIST=$FIXgsm/global_kplist.1d.txt
     ...
     if [ $GRIBVERSION = grib2 ]; then
       export PGBOUT=$COMOUT/pgb${flag}m3.$CDUMP.$CDATE.grib2
     fi
     rm $PGBOUT
     ...
     if [ $GRIBVERSION = grib2 ]; then
       export PGBOUT=$COMOUT/pgb${flag}es.$CDUMP.$CDATE.grib2
     fi
     rm $PGBOUT
     if [ $NCEPPOST = YES -a $KTO -gt 0 ] ; then
     ...
     if [ $GRIBVERSION = grib2 ]; then
       export PGBOUT=$COMOUT/pgb${flag}p3.$CDUMP.$CDATE.grib2
     fi 
     rm $PGBOUT
     if [ $NCEPPOST = YES -a $KTO -gt 0 ] ; then
...
# -------------------
  ...
  export VDATE=$CDATE
  export SIGINP=$SIGIND/${SIGISUF}$SUFOUT
  export NEMSINP=$SIGIND/${SIGISUF}$SUFOUT
  export SFCINPUT=$SIGIND/${SFCISUF}$SUFOUT
  export FLXINP=NULL
  export PGBOUT=$COMOUT/pgb${flag}nl$SUFOUT
  export PGBOUT1=$COMOUT/pgb${flag_pgb}nl$SUFOUT
  if [ $GRIBVERSION = grib2 ]; then
    export PGBOUT=$COMOUT/pgb${flag}nl$SUFOUT.grib2
  fi
  ...
  until [[ $((FH=10#$FH+10#$FHOUTPGB)) -gt $FHEND ]];do [[ $FH -lt 10 ]]&&export FH=0$FH
    ...
    if [ ! -s $COMOUT/pgb${flag_pgb}${FH}${SUFOUT} -o $REDO_POST = YES -a $JUST_AVG = NO ] ; then
      if [[ $CCPOST = YES ]];then
        ...
        export NEMSINP=$datafcst/${SIGOSUF}f${FH}${SUFOUT}
        export SFCINPUT=$datafcst/${SFCOSUF}f${FH}${SUFOUT}
        if [ $NEMSIO_IN = .true. ] ; then
         export LOGINP=$NEMSINP                     
         sleep $tsleep
        fi
        ...
        if [ $NCEPPOST = YES ] ; then
          if [ $IDRT -eq 0  -o $GRID_ID -gt 0 ] ; then
            ...
            if [ $CDUMP = gdas -a ${DO_FLX_INDX:-YES} = YES ] ; then
              $windex $datafcst/${FLXOSUF}f${FH}${SUFOUT} $datafcst/${FLXOSUF}f${FH}${SUFOUT}.index
            fi
            ${precpgb}$COPYGB -g$GRID_ID -i$in_o -x $datafcst/${FLXOSUF}f${FH}${SUFOUT} flxfile
            ...
            export FLXINP=$datafcst/${FLXOSUF}f${FH}${SUFOUT}
            if [ $POSTD3D = YES ] ; then
...
#
        ...
        export NEMSINP=$SIGIND/${SIGOSUF}f${FH}${SUFOUT}
        export SFCINPUT=$SIGIND/${SFCOSUF}f${FH}${SUFOUT}
        nsleep=0
        until [[ -s $SIGINP || $((nsleep+=1)) -gt $msleep ]];do sleep $tsleep;done
        nsleep=0
        until [[ -s $FLXIND/${FLXOSUF}f${FH}${SUFOUT} || $((nsleep+=1)) -gt $msleep ]];do sleep $tsleep;done

        if [ $NCEPPOST = YES ] ; then
          if [ $IDRT -eq 0  -o $GRID_ID -gt 0 ] ; then
            rm flxfile
            ${precpgb}$COPYGB -g$GRID_ID -i$in_o -x $FLXIND/${FLXOSUF}f${FH}${SUFOUT} flxfile
            export FLXINP=flxfile
            if [ $POSTD3D = YES ] ; then
             rm d3dfile
            ...
            export FLXINP=$FLXIND/${FLXOSUF}f${FH}${SUFOUT}
            if [ $POSTD3D = YES ] ; then
...
#
      ...
      if [[ $FFPOST = YES || -s $SIGINP ]];then
        ...
        if [ $GRIBVERSION = grib2 ]; then
          export PGBOUT=$COMOUT/pgb${flag}${FH}${SUFOUT}.grib2
        fi
        rm $PGBOUT
#       export POSTGPLIST=$FIXgsm/global_kplist.1d.txt
        export IGEN=$IGEN_ANL
...
# need to use process ANL ID for F00
        if [ $GRIBVERSION = grib2 ]; then
         ...
         if [[ $CDUMP = gfs ]] ; then
          ...
          then
           ...
           export CTLFILE1=${CTLFILEGFS_f00:-$PARMPOST/postcntrl_gfs.xml}
	  ...
	  then
           ...
           export PostFlatFile=$PARMPOST/postxconfig-NT-GFS.txt
	   export CTLFILE=${CTLFILEGFS:-$PARM_POST/postcntrl_gfs.xml}
	  fi
         fi
        fi
        $POSTGPSH
...
#         $WGRIB $pgbfile|grep -vi :kpds6=100:|${precpgb}$COPYGB -xkw -i$in_o -g$GRID_ID25 $pgbfile pgb25b
         ...
         if [ $GRID_ID62 -gt 0 ] ; then
          ${precpgb}$COPYGB -g$GRID_ID62 -i$in_o -x $FLXIND/${FLXOSUF}f${FH}${SUFOUT} $FLXIND/${FLXOSUF}l${FH}${SUFOUT}
         fi
...
# nsleep=0
# msleep=50
# until [[ -s $COMROT/${SFCOSUF}f06.$CDUMP.$CDATE.LIS || $((nsleep+=1)) -gt $msleep ]];do sleep 10 ; done
