...
# yzdir=${yzdir:-$rdir/save/wx20yz}
  ...
  DMPDIR=${DMPDIR:-$rdir/shared/glopara}
  yzdir=${yzdir:-$rdir/save/Shrinivas.Moorthi/para/wx20yz}
  cdasdir=${cdasdir:-$yzdir/gvrfy/data/cdas}
 ...
 if [ ${TASKGM:-NO} = YES ] ; then
  if [ $COUP_FCST = NO ] ; then
   ...
   if [ $CKSH = fcst -a $JCAP -ge 1500 ] ; then
    ...
    #TASK_GEOMETRY=$(/usrx/local/bin/mktgs 4/4 $((tasks-4))/$((npe_node/NUMTHRD)))
    TASK_GEOMETRY=$(/usrx/local/bin/mktgs 4/4 $((tasks-20))/$((npe_node/NUMTHRD)) 4/4 6/6 6/6)
   fi
  else
